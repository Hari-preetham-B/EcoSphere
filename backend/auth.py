...
def get_supabase_user(token):
    """
    Verifies token against Supabase Auth API.
    Returns user payload dictionary or None if invalid.
    """
    ...
    # Fallback to local JWT decode if API call fails or for local development
    try:
...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        # Sync/fetch user profile in our database
